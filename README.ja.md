...
- 日本語・英語の表示切り替え
- Googleアカウントによる任意のログイン、確認付きゲスト記録取り込み、Solveの継続同期
- PC、タブレット、スマートフォン対応
...
アカウント所有Solveは、ログイン中の端末間で継続的に同期されます。追加、ペナルティ・グループ変更、削除は端末へ即時反映され、オフライン中はFirestoreの再送キューへ保存されます。ヘッダーには同期中、同期済み、オフライン、未同期、エラーを表示し、失敗した操作は再試行できます。FirestoreのWeb永続キャッシュを有効にしているため、信頼できる端末で利用してください。
...
明示的な確認後のゲスト取り込みでは、ログインユーザー本人の`users/{userId}/solves/{solveId}`へ書き込みます。固定UUIDをドキュメントIDに使うため、取り込みの再実行・再試行や継続同期でも記録は重複しません。ログイン中のTimer、履歴、集計は、Firestoreのローカルキャッシュとアカウントの最新スナップショットを統合した記録を使用します。
