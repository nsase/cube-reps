...
ゲスト記録は**アカウント未紐づけ**として扱い、ゲストIDを持ちません。既存のゲストIDは、Solve IDやグループとの関連を保ったまま除去されます。ログイン時の自動取り込みは行いません。履歴で記録を選択し、未紐づけの記録は**現在のアカウントへ移行**、別アカウントの記録は**現在のアカウントへコピー**を確認して実行します。移行はSolve IDを維持し、コピーは新しいIDを作って元記録を変更しません。意図的に再コピーした場合は別の記録が作られます。クラウドへの同期状態はヘッダーで確認してください。
