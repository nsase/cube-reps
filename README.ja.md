...
ログインすると、CubeRepsは端末内のゲスト所有Solve IDとログイン中アカウントのFirestore記録を比較し、アップロードが必要な件数と保存先アカウントを表示します。**アップロード**を選択するまでデータは送信されません。Solve UUIDを使うため再実行しても重複せず、中断時は失敗した記録だけを再試行できます。移行後もローカル記録は保持され、成功した記録はそのアカウント所有として記録されます。

同じUUIDが両方にある場合は、`updatedAt`が新しい方を同期します。同時刻の場合は、内容が異なっていても自動アップロードしません。以後の変更はまだ継続同期されません。ログアウトしても端末内のデータは残ります。ログインとログアウトの操作自体にはインターネット接続が必要です。
