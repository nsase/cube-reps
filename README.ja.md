...
- 「アルゴリズム」メニューからF2L・OLL・PLLの選択画面へ移動できます。広い画面ではサブメニューから直接移動でき、各ページ内でも種類を切り替えられます。F2LもOLL/PLLと共通の画面・カードを使い、41ケースの番号・グループを表示して検索できます。手順の追加・お気に入り・コピー・削除ができ、追加手順とお気に入りは現在のFRスロット用としてローカル保存されます。組み込み手順とSetupはダミーで、キューブ図はSetupから生成した仮の上面図です。ケース固有の手順・図・位置切り替え・計測は未対応です。F2Lのケースキーは2桁の番号とスロットの組み合わせ（例: `F2L-01-FR`）とし、番号を変更する場合は保存済みキーの移行が必要です。
