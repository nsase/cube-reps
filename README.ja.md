...
- 「アルゴリズム」メニューからF2L・OLL・PLLの選択画面へ移動できます。広い画面ではサブメニューから直接移動でき、各ページ内の種類とタイマーのモード（3×3・OLL・PLL）はMaterialボタントグルグループで切り替えられます。F2LもOLL/PLLと共通の画面・カードを使い、41ケースの番号・グループを表示して検索できます。手順の追加・お気に入り・コピー・削除ができ、各カードのMaterialボタントグルグループでFront Right・Front Left・Back Left・Back Rightを切り替えられます。カード内の幅が狭い場合はFR・FL・BL・BRの略称で表示します。Setupはケース共通のFR基準を1つ持ち、FL・BL・BRでは末尾にそれぞれ`y`・`y2`・`y'`を追加します。組み込み手順・追加手順・お気に入りはスロットごとに独立し、追加手順とお気に入りはローカル保存されます。F2Lの図は白下・緑正面のSetup後の状態から、上面・前面・右面を、手前を少し大きく見せる透視投影のクォータービューで表示します。F2Lのピースを見やすくするため、黄色を含むピースは灰色にしています。OLL/PLLは従来の上面図を使用します。全41ケースに[SpeedCubeDB](https://speedcubedb.com/a/3x3/F2L)と[CubeRoot](https://cuberoot.me/alg/3x3/f2l/?orientation=z2)のSetup・手順を反映しています。参照元の番号・名前は各定義のコメントに記載しています。Wide Moveは`Rw`・`Fw`などの`w`表記に統一しています。重複する記法と末尾のy回転を整理し、Setupはすべての掲載手順で解ける外層回転の低コスト候補を採用しています。F2L計測は未対応です。F2Lのケースキーは2桁の番号とスロットの組み合わせ（例: `F2L-01-FR`）とし、番号を変更する場合は保存済みキーの移行が必要です。
