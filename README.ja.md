...
- F2L計測は1ペア単位です。41ケースから固定またはランダムで選び、対象スロット（ランダム・FR・FL・BL・BR）を指定します。ケース・スロットの初期値はどちらもランダムで、それぞれ独立に固定またはランダムを選べます。出題ごとに実際のスロットを決め、記録にはその位置を保持します。白下・緑正面から共通Setupと末尾の持ち替えを回し、対象ペアを解く時間を計測します。スロットだけを変えると現在のケースは維持され、計測中はタイマーだけを表示します。F2L・OLL・PLLの記録は共通の`caseId`・表示番号・スクランブルをローカル保存とクラウド同期で保持し、F2Lでは対象スロットも保持します。再計測は固定ケースIDを優先し、IDのない旧記録はケース名から復元します。履歴ではF2Lカテゴリーで絞り込み・集計・Progress Chartを利用でき、詳細にケースとスロットを表示します。再計測では元のケース・スロット・スクランブルを復元します。
