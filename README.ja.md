...
ログインすると、CubeRepsは端末内のゲスト所有Solve IDとログイン中アカウントのFirestore記録を比較し、アップロードが必要な件数と保存先アカウントを表示します。**アップロード**を選択するまでデータは送信されません。移行開始前にゲスト記録を追加、変更、削除した場合は、表示件数と移行候補が自動的に更新されます。Solve UUIDを使うため再実行しても重複せず、中断時は失敗した記録だけを再試行できます。移行後もローカル記録は保持され、成功した記録はそのアカウント所有として記録されます。
