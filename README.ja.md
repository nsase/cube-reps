...
- 「アルゴリズム」メニューからF2L・OLL・PLLの選択画面へ移動できます。広い画面ではサブメニューから直接移動でき、各ページ内でも種類を切り替えられます。F2LもOLL/PLLと共通の画面でケース検索でき、番号付きの41枚のカードを表示し、各カードにダミーのSolve・Setupを1つずつ用意しています。ケース固有の手順・キューブ図・位置切り替え・計測は未対応です。F2Lのケースキーは2桁の番号とスロットの組み合わせ（例: `F2L-01-FR`）とし、番号を変更する場合は保存済みキーの移行が必要です。
