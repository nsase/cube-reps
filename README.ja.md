...
履歴は、ログイン状態や所有者に関係なく、このブラウザプロファイルに保存した未削除の全Solveを表示します。所有者フィルターは、選択中のグループ・カテゴリー内の一覧、件数、集計、Progress Chartへ一貫して適用されます。所有者アバターは画像、イニシャル、共通アイコンで表示し、タップするとアカウント詳細を確認できます。

ゲスト記録は**アカウント未紐づけ**として扱い、ゲストIDを持ちません。既存のゲストIDは、Solve IDやグループとの関連を保ったまま除去されます。ログイン時の自動取り込みは行いません。履歴で記録を選択し、未紐づけの記録は**現在のアカウントへ移行**、別アカウントの記録は**現在のアカウントへコピー**を確認して実行します。移行はSolve IDを維持し、コピーは新しいIDを作って元記録を変更しません。コピー元は詳細で確認でき、意図的に再コピーした場合は別の記録が作られます。結果通知は履歴内だけに表示され、一定時間で消えます。クラウドへの同期状態はヘッダーで確認してください。
